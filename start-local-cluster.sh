...
start_node() {
    ...
    java \
        ...
        -Draftexchange.liquidation.interval=${LIQ_INTERVAL:-2} \
        -Draftexchange.liquidation.scanSlices=${SCAN_SLICES:-30} \
...
case "${1:-start}" in
    start)
        ...
        esac
        {
            ...
            # 持久化可调项，保证 start-node 重启的节点与集群配置一致（否则 restart 节点会回落默认值）
            echo "SNAPSHOT_MARGIN=${SNAPSHOT_MARGIN:-10000000}"
            echo "LIQ_INTERVAL=${LIQ_INTERVAL:-2}"
            echo "SCAN_SLICES=${SCAN_SLICES:-30}"
            echo "KAFKA_ENABLED=${KAFKA_ENABLED:-false}"
            echo "BATCH_ENABLED=${BATCH_ENABLED:-false}"
        } > "$STATE_FILE"
    ...
    start-node)
        ...
        margin_enabled=true; consensus=jraft
        if [ -f "$STATE_FILE" ]; then
            # shellcheck disable=SC1090
            source "$STATE_FILE"
            [ "$mode" = spot ] && margin_enabled=false || margin_enabled=true
        fi
        start_node "$n" "$margin_enabled" "$consensus"
        ;;
    status)
        if [ -f "$STATE_FILE" ]; then
            # shellcheck disable=SC1090
    ...
    *)
        ...
        echo "       $0 stop-node <1|2|3>   # 停单节点（E2E 故障注入用）"
        echo "       $0 start-node <1|2|3>  # 起单节点（沿用集群 mode/consensus）"
        exit 1
        ;;
esac
