...
2. 清理 `LaneState` 里因 on-lane + apply-guard 而冗余的运行态(`inFlight` / `liqRetryThrottleMs`),连带简化 `submit` 的 `onApplied` 契约,并把 loan 决策时间对齐 `cmd.timestamp`。
...
| 4 | 去 `submit` 的 `onApplied` → `submit(ApiCommand)` | 唯一消费者(loan inFlight 清除)已删;简化 interface + 父类 wrapper + `ExchangeRuntime` override(去 3 处 null-check + 参数) |
...
- `processors/liquidation/LiquidationEngine.java`:`checkPositions` targeted 分支求并 loan 索引;`updateProvider` 委托重建;`checkUser` 传 `ts` 给 loan。
- `processors/liquidation/LiquidationCommandSubmitter.java`:`submit(ApiCommand)`。
- `processors/liquidation/LiquidationScheduledService.java`:`submit` wrapper 去 `onApplied`。
- `processors/loan/LoanCommandHandlers.java`:isolated/cross 生命周期 hook 调用。
- `raft-exchange-server/.../ExchangeRuntime.java`:override lambda 去 `onApplied`。
