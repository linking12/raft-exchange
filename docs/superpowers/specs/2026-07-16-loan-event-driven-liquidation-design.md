...
Plan 2 让 `checkPositions` 在期货活干完后**委托一行** `loanLiquidationEngine.checkLoans(cmd)`。`checkLoans` 用 loan 索引选人:
...
- 可能**多查**(某 uid 的 LTV 实际没动)→ 无害,`check(up,ts)` 跑一遍发现没越线即 no-op;
- 极端多跳估值下可能**漏查**(某 uid 经 `S` 间接受影响却不持 `b`/`q`)→ 由 **backstop 兜底**。

### 2.4 集成:checkLoans 委托,checkUser 纯期货

**定稿架构(单引擎 + 检测入口重构)**:

- `LiquidationEngine` 拥有一个**稳定单例** `LoanLiquidationEngine`(构造函数造一次,`updateProvider` 不再 `new`,恢复时只 `rebuildIndices()`)——这样"检测方(本引擎)"与"维护方(`LoanCommandHandlers` 缓存的 ref)"永远同一实例,快照恢复不分裂。
- `LiquidationEngine.checkUser` 变**纯期货**:删掉原来那句 `loanLiquidationEngine.check(up)`。
- loan 检测独立成 `LoanLiquidationEngine.checkLoans(OrderCommand cmd)`:`cmd.symbol>=0` targeted(`collectAffectedLoanUsers` 查 loan 索引选人 → 逐个 `check(up,ts)`);`cmd.symbol<0` backstop 全扫。
- `checkPositions` 在期货活 + `isRunning()` 门控之后,**委托一行** `loanLiquidationEngine.checkLoans(cmd)`。

loan 的**数据 + 逻辑自洽**在 loan 两类里(索引/维护/检测);只**借**期货引擎的**触发入口**(checkPositions 一行委托)和**基础设施**(provider / submitter / `isRunning`)。期货引擎除这一行委托,不认识 loan 细节。

> 无重复检查:targeted 下 futures 走 `checkUser`、loan 走 `checkLoans`,各查各的;backstop 两条各遍历一次全量(冷路径,可忽略)。
...
**生产**
- `processors/loan/LoanLiquidationEngine.java`:新增检测入口 `checkLoans(cmd)`(targeted/backstop);加两索引 + 维护/查询/重建 helper;删 `inFlight`/`liqRetryThrottleMs`/`stuckLiqThrottled`,`LaneState` 塌缩;`check(up, ts)`;force-sell/margin-call `submit(cmd, null)`。
- `processors/liquidation/LiquidationEngine.java`:loan 引擎**单例化**(ctor 造、updateProvider 不再 new);`checkPositions` 末尾**委托一行** `loanLiquidationEngine.checkLoans(cmd)`;`checkUser` **删** `loan.check()`(纯期货);`updateProvider` 加 `rebuildIndices()`。
- `processors/loan/LoanCommandHandlers.java`:缓存 `loanLiquidationEngine` 稳定单例;isolated/cross 生命周期 hook 调用。

> 不改:`RiskEngine` / `LiquidationCommandSubmitter` / `LiquidationScheduledService` / `ExchangeRuntime`。
