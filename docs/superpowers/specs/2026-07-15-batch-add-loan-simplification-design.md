...
- **`SpotLoanRateCurveConfig`**:只留一个 `SpotLoanRatePreset preset` 字段(CONSERVATIVE/STANDARD/AGGRESSIVE,UNSPECIFIED→STANDARD;kink 固定 80%)。custom 曲线也不暴露(exchange-core `ofRateCurve` 仍支持,proto 需要时再加),贯彻"最小面、按需再加"。
