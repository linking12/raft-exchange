...
crossMarginCall= 指定值(override) 否则 crossLiquidationLtv − global.ltvMarginCallBufferBps
...
- `GlobalLoanConfig.thresholdsValidGivenCurrent(...)`:纳入两个新缓冲的合法性(`buffer > 0`,派生出的 cross 阈值满足 `marginCall < liquidation < 100%`)。
...
- **Global 序列化 + LoanGlobalConfig 快照格式变更**(+2 缓冲 int):这是本设计唯一的破坏性变更点。
  ...
  - 处理策略(择一,建议 A):
    - **A(推荐,pre-launch):** 当前处于上线前(尚无生产快照),直接改格式,接受 stateHash 变化,恢复走全量 clean start / 重放;无需版本兼容代码。
    - B(若已有生产快照):给 `LoanGlobalConfig` 序列化加版本位,旧版本缺失字段读默认缓冲。
- 新旧工厂并存;运营方逐步迁到 `ofMarket`/预设。

**决策点:** 采用 A 的前提是"loan 子系统尚未有需要保留的生产快照"。若不成立,改用 B(加版本位),实现计划相应增加一步。
