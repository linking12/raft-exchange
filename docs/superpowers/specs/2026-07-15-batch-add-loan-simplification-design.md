...
- **`SpotLoanConfig`**:`symbolId`+`loanInitialLtvBps` 必填;5 个 override 字段改 proto3 `optional`(field presence),缺省 → converter 映射为 `SymbolLoanConfig.UNSET(−1)` → exchange-core `resolve()` 派生。
