...
- `LiquidationFlow.java` — 单个强平流程的 leader-local 状态(stage/size/originalOrderId/lastTransitionAt)。
- `LiquidationFlowTracker.java` — `IdentityHashMap<SymbolPositionRecord, LiquidationFlow>` + FORCE→IF→ADL 升级决策 + stuck 扫描。
- `FuturesSolvencyEvaluator.java` — 期货 isolated/cross 偿付判定 + BP/size,返回 verdict(**不 publish**)。
- `LiquidationBackstopCursor.java` — 兜底切片游标(uid 快照 + cursor)。
- `LiquidationHeartbeatService.java` — `extends SimpleScheduledService`,只 publish `LIQUIDATION_BACKSTOP_TICK`(慢)+ `REPRICE_LOAN_RATES`(1h),不碰用户态。
...
- `processors/liquidation/LiquidationEngine.java` — 去 `SimpleScheduledService`;薄编排;新 API `checkPositionsOnSymbol/checkUser/fullScan/backstopTick/onPositionOpened/onPositionClosed/rebuildIndex/nextLiquidationState`;持有 index/tracker/evaluator/cursor + `BooleanSupplier isLeader`。
- `processors/liquidation/LiquidationContext.java` — 保留(内存态,给 `LiquidationFlow` 复用 enum);不再被 `SymbolPositionRecord` 引用。
...
## Task 3: LiquidationFlow + LiquidationFlowTracker(leader-local 流程状态机)

**Files:**
- Create: `exchange-core/src/main/java/exchange/core2/core/processors/liquidation/LiquidationFlow.java`
...
**背景:** 把原来挂在 `SymbolPositionRecord.liquidationCtx` 上的强平流程状态搬到引擎持有的 leader-local 结构。`LiquidationFlow` 复用 `LiquidationContext.LiquidationState` enum(LIQUIDATING/WAIT_IF_EXECUTION/WAIT_ADL_EXECUTION)。Tracker 按 `SymbolPositionRecord` 对象身份键入(`IdentityHashMap`),流程完结或仓位删除时移除条目。

**Interfaces:**
- `LiquidationFlow`:字段 `LiquidationContext.LiquidationState state; long price; long size; long originalOrderId; long lastTransitionAt;` + 构造 `LiquidationFlow(long price, long size, long originalOrderId, long ts)`(初始 `state=LIQUIDATING`)。
- `LiquidationFlowTracker`:
  - `LiquidationFlow get(SymbolPositionRecord pos)` — 无则 null。
  - `boolean hasActiveFlow(SymbolPositionRecord pos)` — `get(pos) != null`。
  - `LiquidationFlow begin(SymbolPositionRecord pos, long price, long size, long originalOrderId, long ts)` — 新建并登记。
  - `void touch(LiquidationFlow flow, long ts)` — 刷新 `lastTransitionAt`。
  - `void remove(SymbolPositionRecord pos)` — 流程完结/仓位删除时清除。
  - `void forEachStuck(long now, long thresholdMs, java.util.function.BiConsumer<SymbolPositionRecord, LiquidationFlow> action)` — 遍历 `now - lastTransitionAt > thresholdMs` 的条目(用于兜底 republish)。遍历时快照 entrySet 避免并发修改。
  - `void clear()` / `int size()`。

**Interfaces(Produces,后续 Task 6 消费):** 上述全部。

- [ ] **Step 1: 写失败测试**(节选核心用例;实现者补全)
...
class LiquidationFlowTrackerTest {
    ...
    private SymbolPositionRecord pos() {
        return new SymbolPositionRecord(); // 无参构造后 initialize 前即可作 identity key;若无无参构造,用 mock
    }
    ...
    void beginThenGet_tracksFlow() {
        ...
        LiquidationFlow f = t.begin(p, 100L, 5L, 42L, 1000L);
    ...
    void forEachStuck_onlyBeyondThreshold() {
        ...
        t.forEachStuck(12_000L, 5_000L, (p, f) -> hit.add(p)); // now-last: fresh=2000<=5000 跳过, stale=11000>5000 命中
    ...
    void touch_refreshesTimestamp_movesOutOfStuck() {
        LiquidationFlowTracker t = new LiquidationFlowTracker();
        SymbolPositionRecord p = pos();
        LiquidationFlow f = t.begin(p, 1, 1, 1, 1_000L);
        t.touch(f, 12_000L);
        List<SymbolPositionRecord> hit = new ArrayList<>();
...
```
> 注:若 `SymbolPositionRecord` 无可用无参构造,`pos()` 改为 `org.mockito.Mockito.mock(SymbolPositionRecord.class)`——identity key 用 mock 即可。实现者按实际构造情况二选一。

- [ ] **Step 2:** 跑失败(`-Dtest=LiquidationFlowTrackerTest`)。

- [ ] **Step 3: 实现**

`LiquidationFlow.java`:
```java
package exchange.core2.core.processors.liquidation;

import exchange.core2.core.processors.liquidation.LiquidationContext.LiquidationState;

/** 单个强平流程的 leader-local 状态(替代原 SymbolPositionRecord.liquidationCtx)。不进 snapshot/stateHash。 */
public final class LiquidationFlow {
    ...
    public LiquidationFlow(long price, long size, long originalOrderId, long ts) {
...
/**
 * leader-local 强平流程追踪器:SymbolPositionRecord 身份 -> LiquidationFlow。
...
public final class LiquidationFlowTracker {
    ...
    private final Map<SymbolPositionRecord, LiquidationFlow> flows = new IdentityHashMap<>();

    public LiquidationFlow get(SymbolPositionRecord pos) {
    ...
    public LiquidationFlow begin(SymbolPositionRecord pos, long price, long size, long originalOrderId, long ts) {
        final LiquidationFlow flow = new LiquidationFlow(price, size, originalOrderId, ts);
        flows.put(pos, flow);
        return flow;
    }

    public void touch(LiquidationFlow flow, long ts) {
    ...
    public void forEachStuck(long now, long thresholdMs, BiConsumer<SymbolPositionRecord, LiquidationFlow> action) {
        if (flows.isEmpty()) {
            return;
        }
        // 快照 entrySet:action 可能触发 begin/remove(republish 路径),避免并发修改。
        final List<Map.Entry<SymbolPositionRecord, LiquidationFlow>> snapshot = new ArrayList<>(flows.entrySet());
        for (Map.Entry<SymbolPositionRecord, LiquidationFlow> e : snapshot) {
...
- [ ] **Step 5:** 提交 `LiquidationFlow.java` + `LiquidationFlowTracker.java` + 测试。
...
**Files:**
- Create: `exchange-core/src/main/java/exchange/core2/core/processors/liquidation/FuturesSolvencyEvaluator.java`
- Create: `exchange-core/src/main/java/exchange/core2/core/processors/liquidation/LiquidationAction.java`(小 verdict 值对象)
...
**Interfaces:**
- `LiquidationAction`:`final` 值对象,字段 `final SymbolPositionRecord position; final long bankruptcyPrice; final long size; final boolean warning;`(warning=true 表示只需预警不需强平,此时 bp/size 可为 0)。加静态工厂 `liquidate(pos, bp, size)` / `warn(pos)`。
...
**背景:** `liquidationCtx` 当前进 snapshot + stateHash,且 64-66 有注释声称"必须持久化否则发散"。本计划按 spec §8 反转该决定:流程状态改为 leader-local(Task 3),正确性由 size 夹取 + 冷启动重扫保证。此处**删字段与其序列化/哈希参与**。

**Interfaces:**
- Produces:`SymbolPositionRecord` 不再有 `liquidationCtx` 字段;`writeMarshallable`/读构造/`stateHash` 少一项。`LiquidationContext` 类保留(内存态,给 `LiquidationFlow` 复用 enum)。
...
- [ ] **Step 3: 实现删除**(逐处):
  ...
  - 删无用 import(`LiquidationContext` 若不再被引用)。
...
- 去掉 `extends SimpleScheduledService`;类改为普通 `final class`。构造签名不变(`Supplier<FundEvent>, int shardId, ExchangeConfiguration`)——但不再调 `super(...)` 定时器。
...
- 删除:`checkLiquidations()`、`runOneIteration()`、`triggerOnce()`、`tryRepublishStuckLiquidation()`(逻辑并入 `backstopTick` 的 `republishStuck`)、`inFlightLiquidationCmd` 字段及其 `publishTracked` 里的 add/remove(见下)、`checkLiquidationIsolated/Cross`/`forceCrossLiquidation`/`calculateBankruptcyPrice`/`calculateCrossBpMarginBaseAllocation`(已迁 evaluator)、`start()` override。
- **loan 保留旧路**:`fullScan`/`backstopTick`/`checkUserInternal` 中仍调用 `loanLiquidationEngine.check(up)`(loan 事件化是 Plan 2)。**但** reprice 心跳从 `LoanLiquidationEngine.check` 里移除(改由 Task 9 的 `LiquidationHeartbeatService` 发)——见 Task 9;本任务先保留 loan.check 原状,Task 9 再摘 reprice。
...
## Task 9: 定时器收缩(轻心跳只发命令)+ server 换届接线

**Files:**
- Create: `exchange-core/src/main/java/exchange/core2/core/processors/liquidation/LiquidationHeartbeatService.java`
- Modify: `exchange-core/src/main/java/exchange/core2/core/processors/loan/LoanLiquidationEngine.java`(摘除 reprice 心跳块 67-77 中的 reprice 发送)
- Modify: `exchange-core/src/main/java/exchange/core2/core/ExchangeCore.java`(暴露/构造 heartbeat + `getLiquidationEngines()` 已有)
- Modify: server 层 `ExchangeRuntime` / `JraftClusterContainer`(85-91)/ `AeronClusterContainer`(66-72):换届 LEADER→`heartbeat.start()` + 各引擎 `fullScan()` + `setIsLeader(...)`;非 LEADER→`heartbeat.stop()`。
- Test: `exchange-core/src/test/java/exchange/core2/core/processors/liquidation/LiquidationHeartbeatServiceTest.java`

**Interfaces:**
- `LiquidationHeartbeatService extends SimpleScheduledService`:构造持 `LiquidationCmdPublisher publisher`(或持 shard-0 `LiquidationEngine` 以取 publisher)+ `int shardId` + 间隔(基 tick,如 1s)。`runOneIteration()`:仅 `shardId==0` 时——按 `BACKSTOP_INTERVAL_MS`(默认 30_000,sysprop `raftexchange.liquidation.backstopIntervalMs`)节流 publish `ApiLiquidationBackstopTick.builder().build()`;按 `REPRICE_INTERVAL_MS`(1h)节流 publish `ApiRepriceLoanRates.builder().build()`。**不碰任何用户态**。用 `System.currentTimeMillis()` 做节流(纯 leader-local 调度,不进复制态,允许 wall-clock)。
- `LoanLiquidationEngine.check`:删除 67-77 里的 reprice 发送块(reprice 改由 heartbeat 发);`check` 只留 isolated/cross loan 扫描(仍被 `LiquidationEngine.fullScan/backstopTick/checkUserInternal` 的 loan 旧路调用)。

- [ ] **Step 1: 写失败测试** —— `LiquidationHeartbeatServiceTest`:mock `LiquidationCmdPublisher`,手动调 `runOneIteration()` 多次 + 操纵时间(把间隔常量设小或用可注入的 clock——**决策**:构造注入 `LongSupplier clock` 便于测试,生产传 `System::currentTimeMillis`),断言:shard 0 第一次发 BACKSTOP_TICK;间隔内重复调不重发;超 REPRICE 间隔发 REPRICE;`shardId != 0` 一律不发。

- [ ] **Step 2:** 跑失败。
- [ ] **Step 3: 实现** heartbeat 类 + 从 `LoanLiquidationEngine.check` 摘 reprice + server 换届接线(LEADER: start heartbeat、`engines.forEach(e -> { e.setIsLeader(isLeaderSupplier); e.fullScan(); })`;非 leader: stop heartbeat)。删除旧的"`engines.forEach(SimpleScheduledService::start)`"——`LiquidationEngine` 已非 `SimpleScheduledService`;改为启停 `heartbeat` 并触发 `fullScan`。
- [ ] **Step 4: 跑测试**
```bash
mvn -pl exchange-core -Dtest=LiquidationHeartbeatServiceTest,LoanLiquidationEngineTest test
```
Expected: PASS(注意:`LoanLiquidationEngineTest` 里若有断言 reprice 由 check 发出的用例,需同步迁移到 heartbeat 测试或调整)。
- [ ] **Step 5:** 提交。

> **server 层编译**:改 `JraftClusterContainer`/`AeronClusterContainer` 后需 `mvn -pl raft-exchange-server -am compile` 确认过编(参考既有教训:改引擎 API 要全仓 grep 下游)。
...
- **无竞态**(结构性):删除 off-lane 扫描线程后,无独立线程读 `UserProfile`(`LiquidationEngine` 不再 `extends SimpleScheduledService`;唯一定时器 `LiquidationHeartbeatService` 不碰用户态)。
...
5. **loan 侧本计划保留旧扫描路**(经 `loanLiquidationEngine.check(up)` 挂在 futures 检查里),仅摘走 reprice 心跳到 `LiquidationHeartbeatService`。loan 的 targeted 事件化是 **Plan 2**。
