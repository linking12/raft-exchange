...
- [ ] **Step 3: 实现:**
  ...
  - 新/改方法(时间戳一律用入参 `cmd.timestamp`):
    ...
    - `backstopTick(OrderCommand cmd)`:`if(!isRunning())return;` 切片(**兼冷启动**:leader 定时器 start 后每 tick 推进,一轮 sweep 覆盖全部):游标取 `userProfileService.getUserProfiles().keySet().toArray()` 快照,每 tick 扫 N 个调 `checkUser(up, cmd.timestamp)`,扫到尾归零 + 换快照。**无 stuck 分支**。
