 ...
 *
 * <p>每个 shard 装 1 个 {@link PerShardData}（4 个 per-currency 桶）；{@link #merge} 后 byShard 装全部 N 个 shard。
 * 4 桶口径完全对齐 {@link LoanService} 的 raft snapshot 字段：
 * <ul>
 *   <li>{@code interestRevenue} —— 累计利息收入（loanCurrency scale）</li>
 *   <li>{@code loanInsuranceFund} —— LIF 保险基金余额（可为负，负值即平台已垫资额；水位告警的数据源，见 loan.md §18.10）</li>
