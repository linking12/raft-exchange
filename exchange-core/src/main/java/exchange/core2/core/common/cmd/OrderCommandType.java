...
public enum OrderCommandType {
    ...
    INTERNAL_TRANSFER((byte) 14, true),
    ...
    LOAN_FORCE_LIQUIDATE((byte) 54, true),            
    ...
    LOAN_CROSS_FORCE_LIQUIDATE((byte) 59, true),     
    POOL_DEPOSIT((byte) 60, true),
    POOL_WITHDRAW((byte) 61, true),
    REPRICE_LOAN_RATES((byte) 63, true),
    LOAN_IF_DEPOSIT((byte) 64, true),
    LOAN_IF_WITHDRAW((byte) 65, true),
    ...
    /**
     * 非交易命令判断 —— 与 {@link #isLoan()} 同理做二级 dispatch 门守：命中则整块委托给
     * {@code RiskEngineCommandDispatcher.dispatch}，主 switch 只留交易（下单）/ 结算 / 引擎自身生命周期。
     * 涵盖建用户 / 账户充提 / 保证金 / 杠杆 / 持仓模式 / 挂起恢复 / 标记价 / 保险基金充提，
     * 以及运营命令：手续费重置 / 借贷利率重定价 / BINARY_DATA 批量配置。
     */
    public boolean isNonTrading() {
        switch (this) {
            // 账户维度
            case ADD_USER:
     ...
     * 命中则整块委托给 {@code LoanCommandDispatcher.dispatch}，主 switch 里永远看不到 loan 命令。
     * 详见 loan.md。
     */
