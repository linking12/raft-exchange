...
public final class SingleUserReportResult implements ReportResult {
    ...
    /**
     * 现货侧冻结资产（按 currency 聚合）：未成交挂单等占用的部分。 客户端按 currency 算"可支配余额"： available[c] = accounts[c] - exchangeLocked[c] -
     * Σ_{p: p.currency==c} (p.openInitMarginSum + p.pendingMargin + p.fee)
     */
    private final IntLongHashMap exchangeLocked;
    /**
     * symbol -> positions list: [LONG] always comes before [SHORT] if both exist; if only one position exists, it can
     * be either LONG or SHORT.
     */
    private final IntObjectHashMap<List<Position>> positions;

    // risk engine: 现货借贷持仓（loan）。三个引用字段 null = 该 shard 无此用户。
    /** Isolated 逐仓 loan：每笔 1:1 抵押，自带 per-loan 实时 LTV。 */
    private final List<IsolatedLoan> isolatedLoans;
    /** Cross 全仓 loan：单笔不含抵押/LTV，抵押与健康度都在账户级（见下 crossLoanCollateral / crossAccountLtvBps）。 */
    ...
    /** Cross 账户级 LTV（bps）= Σ债务 ÷ Σ抵押估值（folded 到 numeraire）；numeraire 未配 / 无 cross loan → 0。 */
    ...
    public static SingleUserReportResult createFromMatchingEngine(long uid, IntObjectHashMap<List<Order>> orders) {
        return new SingleUserReportResult(uid, null, null, null, null, null, null, null, 0L, orders,
            QueryExecutionStatus.OK);
    }

    public static SingleUserReportResult createFromRiskEngineFound(long uid, UserStatus userStatus,
        IntLongHashMap accounts, IntLongHashMap exchangeLocked, IntObjectHashMap<List<Position>> positions,
        List<IsolatedLoan> isolatedLoans, List<CrossLoan> crossLoans, IntLongHashMap crossLoanCollateral,
        long crossAccountLtvBps) {
        return new SingleUserReportResult(uid, userStatus, accounts, exchangeLocked, positions, isolatedLoans,
            crossLoans, crossLoanCollateral, crossAccountLtvBps, null, QueryExecutionStatus.OK);
    }

    public static SingleUserReportResult createFromRiskEngineNotFound(long uid) {
        return new SingleUserReportResult(uid, null, null, null, null, null, null, null, 0L, null,
    ...
    private SingleUserReportResult(final BytesIn bytesIn) {
        ...
        this.positions = bytesIn.readBoolean()
            ? SerializationUtils.readIntHashMap(bytesIn, b -> SerializationUtils.readList(b, Position::new)) : null;
        ...
        this.orders = bytesIn.readBoolean()
            ? SerializationUtils.readIntHashMap(bytesIn, b -> SerializationUtils.readList(b, Order::new)) : null;
    ...
    public static SingleUserReportResult merge(final Stream<BytesIn> pieces) {
        return pieces.sequential() // 强制串行流，不用ForkJoinPool
            .map(SingleUserReportResult::new).reduce(IDENTITY, (a, b) -> new SingleUserReportResult(
    ...
    public String toString() {
        return "SingleUserReportResult{" + "userStatus=" + userStatus + ", accounts=" + accounts + ", exchangeLocked="
            + exchangeLocked + ", isolatedLoans=" + isolatedLoans + ", crossLoans=" + crossLoans
            + ", crossLoanCollateral=" + crossLoanCollateral + ", crossAccountLtvBps=" + crossAccountLtvBps
            + ", orders=" + orders + ", queryExecutionStatus=" + queryExecutionStatus + '}';
    }
}
