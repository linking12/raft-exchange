...
public interface LoanRecord {
    ...
    long getAccSnapshot(); // FLOATING 计息游标：上次 accrue 的 liveAcc 快照（bps·ms）

    void setAccSnapshot(long value);

    boolean isFixedRate(); // true=Fixed 走线性计息 / false=Floating 走累加器；Cross 恒 false

    long getCumInterestPaid(); // 累计已付利息（单调递增）；FundEvent 只发快照，本次量由相邻两条相减

    void setCumInterestPaid(long value);
}
