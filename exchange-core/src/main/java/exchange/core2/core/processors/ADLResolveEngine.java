...
/**
 * ADLResolveEngine is responsible for resolving auto-deleveraging execution
...
public class ADLResolveEngine {
    ...
    private void resolveADLExecution(OrderCommand cmd) {
        long remaining = cmd.size;
        if (remaining <= 0) {
            return;
        }
        final ADLCandidate[] candidatesByShard = cmd.adlCandidatesByShard;
        if (candidatesByShard == null || candidatesByShard.length == 0) {
            return;
        }

        // 每个 shard 一个 cursor，指向该 shard 的当前 candidate
        final ADLCandidate[] cursors = candidatesByShard.clone(); // 浅拷贝
        MatcherTradeEvent head = null;
        MatcherTradeEvent tail = null;
        while (remaining > 0) {
            ADLCandidate best = null;
            int bestShard = -1;

            // 在所有 shard 的当前 candidate 中选 score 最大的
            for (int i = 0; i < cursors.length; i++) {
                final ADLCandidate c = cursors[i];
                if (c == null) {
                    continue;
                }
            ...
            if (execSize == best.volume) {
                // 用完了，切到下一个
                cursors[bestShard] = best.nextCandidate;
            } else {
