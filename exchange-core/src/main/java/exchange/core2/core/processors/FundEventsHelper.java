...
public class FundEventsHelper {
    ...
    /**
     * 内部转账余额快照（付款方 / 收款方共用）。两腿同落 taker 桶：付款方在 R1 写、收款方在 R2 写，
     * 隔着 pipeline stage 屏障串行，不存在多 shard 并发写同一链的竞争，无需拆 maker 桶。
     */
    public FundEvent sendInternalTransferEvent(OrderCommand cmd, long uid, int currency, long free, long locked) {
        FundEvent event = buildSpotEvent(cmd.orderId, uid, FundEventType.INTERNAL_TRANSFER, currency, free, locked);
        addFundEvent(cmd, cmd.orderId, event);
        return event;
    }
    ...
    /**
     * loan 事件骨架：设身份 + 两侧币种 scale + 两侧余额；快照/增量字段由各 send 方法按 eventType 填。
     *
     * <p>借款币侧余额走通用 {@code free}/{@code locked}（绑 {@code event.currency}，口径同现货/期货）；
     * 抵押币侧另走 {@code loanCollateralFree}/{@code loanCollateralLocked}（抵押币 scale 与借款币不同，单独下发）。
     * Cross 的加/减抵押无唯一借款币（loanCurrency=0），借款币侧 scale 与余额均置 0。
     */
    private FundEvent newLoanEvent(long loanId, long uid, byte loanMode, int loanCurrency, int collateralCurrency,
        FundEventType type, long free, long locked, long collateralFree, long collateralLocked) {
        FundEvent event = newFundEvent();
        ...
        event.free = loanCurrency == 0 ? 0 : free;
        event.locked = loanCurrency == 0 ? 0 : locked;
        event.loanCollateralCurrencyScaleK = collateralCurrency == 0 ? 0
            : currencySpecificationProvider.getCurrencySpecification(collateralCurrency).getCurrencyScaleK();
        event.loanCollateralFree = collateralFree;
        event.loanCollateralLocked = collateralLocked;
        return event;
    }
    ...
    public FundEvent sendLoanBorrowEvent(OrderCommand cmd, long uid, long loanId, byte loanMode, int loanCurrency,
        int collateralCurrency, long debtPrincipal, long collateralPledged, long ltvBps, long free, long locked,
        long collateralFree, long collateralLocked) {
        FundEvent event = newLoanEvent(loanId, uid, loanMode, loanCurrency, collateralCurrency,
            FundEventType.LOAN_BORROW, free, locked, collateralFree, collateralLocked);
        event.loanDebtPrincipal = debtPrincipal;
        event.loanCollateralPledged = collateralPledged;
    ...
    public FundEvent sendLoanRepayEvent(OrderCommand cmd, long uid, long loanId, byte loanMode, int loanCurrency,
        int collateralCurrency, long debtPrincipal, long debtInterest, long collateralPledged, long ltvBps,
        long interestPaidTotal, long free, long locked, long collateralFree, long collateralLocked) {
        FundEvent event = newLoanEvent(loanId, uid, loanMode, loanCurrency, collateralCurrency,
            FundEventType.LOAN_REPAY, free, locked, collateralFree, collateralLocked);
        event.loanDebtPrincipal = debtPrincipal;
        event.loanDebtInterest = debtInterest;
        event.loanCollateralPledged = collateralPledged;
        event.loanLtvBps = ltvBps;
        event.loanInterestPaidTotal = interestPaidTotal;
        addFundEvent(cmd, cmd.orderId, event);
        return event;
    }
    ...
    public FundEvent sendLoanCollateralChangeEvent(OrderCommand cmd, long uid, long loanId, byte loanMode,
        int loanCurrency, int collateralCurrency, long debtPrincipal, long debtInterest, long collateralPledged,
        long ltvBps, long free, long locked, long collateralFree, long collateralLocked) {
        FundEvent event = newLoanEvent(loanId, uid, loanMode, loanCurrency, collateralCurrency,
            FundEventType.LOAN_COLLATERAL_CHANGE, free, locked, collateralFree, collateralLocked);
        event.loanDebtPrincipal = debtPrincipal;
        event.loanDebtInterest = debtInterest;
        event.loanCollateralPledged = collateralPledged;
        event.loanLtvBps = ltvBps;
    ...
    /**
     * LOAN_LIQUIDATED：强平核销（force-sell 结算：抵押卖出、抵债、overpay 退回；卖不掉则 LIF 接管）。
    ...
    public FundEvent sendLoanLiquidatedEvent(OrderCommand cmd, long uid, long loanId, byte loanMode, int loanCurrency,
        ...
        long interestPaidTotal, long free, long locked, long collateralFree, long collateralLocked) {
        FundEvent event = newLoanEvent(loanId, uid, loanMode, loanCurrency, collateralCurrency,
            FundEventType.LOAN_LIQUIDATED, free, locked, collateralFree, collateralLocked);
        event.loanDebtPrincipal = debtPrincipal;
        event.loanDebtInterest = debtInterest;
        event.loanCollateralPledged = collateralPledged;
        event.loanLtvBps = ltvBps;
    ...
    public FundEvent sendLoanMarginCallEvent(long uid, long loanId, byte loanMode, int loanCurrency, long ltvBps,
        long thresholdBps) {
        // 纯预警、scanner 触发（拿不到 RiskEngine 算 locked），余额侧一律 0
        FundEvent event =
            newLoanEvent(loanId, uid, loanMode, loanCurrency, 0, FundEventType.LOAN_MARGIN_CALL, 0, 0, 0, 0);
        event.loanLtvBps = ltvBps;
