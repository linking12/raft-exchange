...
public final class DisruptorExceptionHandler<T> implements ExceptionHandler<T> {
    ...
    public void handleEventException(Throwable ex, long sequence, T event) {
        ...
        log.error("Disruptor '{}' seq={} caught exception, event={}", name, sequence, event, ex);
        onException.accept(ex, sequence);
    }

    @Override
    public void handleOnStartException(Throwable ex) {
        log.error("Disruptor '{}' startup exception", name, ex);
    }

    @Override
    public void handleOnShutdownException(Throwable ex) {
        log.warn("Disruptor '{}' shutdown exception", name, ex);
    }
}
