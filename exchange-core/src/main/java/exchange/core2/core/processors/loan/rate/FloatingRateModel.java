 ...
 * 借贷，也是 {@link FixedRateModel} 开仓锁率的来源。per-shard 复制状态。
 *
 * <p>计息用"累加器差值法"：{@code accRateBpsMs} 按币种累积"利率 × 时间"，只在 reprice 时用旧生效利率
...
public final class FloatingRateModel implements WriteBytesMarshallable, StateHash {
    ...
    public static long utilizationBps(long borrowed, long available) {
        long total = Math.addExact(borrowed, available);
    ...
    public void advanceAccumulator(int currency, long tickTs) {
        if (lastRepriceTs > 0 && tickTs > lastRepriceTs) {
            ...
            long advance = Math.multiplyExact((long)currentRateBpsOrBase(currency), elapsed);
            accRateBpsMs.put(currency, Math.addExact(accRateBpsMs.get(currency), advance));
        }
    }
    ...
    public long liveAccRateBpsMs(int currency, long now) {
        ...
        if (lastRepriceTs <= 0 || elapsed <= 0) {
            return acc;
        }
        return Math.addExact(acc, Math.multiplyExact((long)currentRateBpsOrBase(currency), elapsed));
    }
    ...
    private static long pending(LoanRecord loan, long liveAcc) {
        long deltaAcc = Math.subtractExact(liveAcc, loan.getAccSnapshot());
