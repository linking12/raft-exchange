...
public final class ExchangeApi {
    ...
    public void submitCommand(ApiCommand cmd) {
        ...
        } else if (cmd instanceof ApiInternalTransfer) {
            ringBuffer.publishEvent(INTERNAL_TRANSFER_TRANSLATOR, (ApiInternalTransfer)cmd);
        ...
        } else if (cmd instanceof ApiLoanIfDeposit) {
            ringBuffer.publishEvent(LOAN_IF_DEPOSIT_TRANSLATOR, (ApiLoanIfDeposit)cmd);
        } else if (cmd instanceof ApiLoanIfWithdraw) {
            ringBuffer.publishEvent(LOAN_IF_WITHDRAW_TRANSLATOR, (ApiLoanIfWithdraw)cmd);
    ...
    public CompletableFuture<CommandResultCode> submitCommandAsync(ApiCommand cmd) {
        if (cmd instanceof ApiMoveOrder) {
            ...
            return submitCommandAsync(INTERNAL_TRANSFER_TRANSLATOR, (ApiInternalTransfer)cmd);
        } else if (cmd instanceof ApiResumeUser) {
            ...
            return submitCommandAsync(LOAN_IF_DEPOSIT_TRANSLATOR, (ApiLoanIfDeposit)cmd);
        } else if (cmd instanceof ApiLoanIfWithdraw) {
            return submitCommandAsync(LOAN_IF_WITHDRAW_TRANSLATOR, (ApiLoanIfWithdraw)cmd);
        } else if (cmd instanceof ApiBinaryDataCommand) {
    ...
    public CompletableFuture<OrderCommand> submitCommandAsyncFullResponse(ApiCommand cmd) {
        if (cmd instanceof ApiMoveOrder) {
            ...
            return submitCommandAsyncFullResponse(INTERNAL_TRANSFER_TRANSLATOR, (ApiInternalTransfer)cmd);
        } else if (cmd instanceof ApiResumeUser) {
            ...
            return submitCommandAsyncFullResponse(LOAN_IF_DEPOSIT_TRANSLATOR, (ApiLoanIfDeposit)cmd);
        } else if (cmd instanceof ApiLoanIfWithdraw) {
            return submitCommandAsyncFullResponse(LOAN_IF_WITHDRAW_TRANSLATOR, (ApiLoanIfWithdraw)cmd);
        } else if (cmd instanceof ApiBinaryDataCommand) {
    ...
    private static final EventTranslatorOneArg<OrderCommand, ApiLiquidationScan> LIQUIDATION_SCAN_TRANSLATOR = (cmd, seq, api) -> {
        ...
        cmd.resultCode = CommandResultCode.NEW;
        cmd.symbol = -1;
        // 轮转分片随命令走：uid = 分片号、size = 总分片数（沿用 IF 运维命令用 cmd.uid 传 shard 的约定）
        cmd.uid = api.scanSlice;
        cmd.size = api.sliceCount;
    };
    ...
    private static final EventTranslatorOneArg<OrderCommand, ApiInternalTransfer> INTERNAL_TRANSFER_TRANSLATOR =
        (cmd, seq, api) -> {
            cmd.command = OrderCommandType.INTERNAL_TRANSFER;
            cmd.orderId = api.transactionId;
            cmd.symbol = api.currency;
            cmd.uid = api.fromUid;
            cmd.size = api.toUid; // size 承载 toUid
            cmd.price = api.amount;
            cmd.timestamp = api.timestamp;
            cmd.resultCode = CommandResultCode.NEW;
        };
    ...
    // ============================== loan 子域 translators（详见 loan.md §5 + LoanCommandDispatcher 头部 field 映射约定）==============================
    ...
    private static final EventTranslatorOneArg<OrderCommand, ApiLoanIfDeposit> LOAN_IF_DEPOSIT_TRANSLATOR =
        (cmd, seq, api) -> {
            cmd.command = OrderCommandType.LOAN_IF_DEPOSIT;
            cmd.timestamp = api.timestamp;
            cmd.uid = api.shardId;
            cmd.symbol = api.currency;
            cmd.size = api.amount;
    ...
    private static final EventTranslatorOneArg<OrderCommand, ApiLoanIfWithdraw> LOAN_IF_WITHDRAW_TRANSLATOR =
        (cmd, seq, api) -> {
            cmd.command = OrderCommandType.LOAN_IF_WITHDRAW;
            cmd.timestamp = api.timestamp;
            cmd.uid = api.shardId;
            cmd.symbol = api.currency;
            cmd.size = api.amount;
    ...
    static {
        ...
        m.put(ApiInternalTransfer.class, INTERNAL_TRANSFER_TRANSLATOR);
        ...
        m.put(ApiLoanIfDeposit.class, LOAN_IF_DEPOSIT_TRANSLATOR);
        m.put(ApiLoanIfWithdraw.class, LOAN_IF_WITHDRAW_TRANSLATOR);
