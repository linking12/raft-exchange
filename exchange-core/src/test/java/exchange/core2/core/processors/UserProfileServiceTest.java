...
class UserProfileServiceTest {
    ...
    private static final long NOW = 1_000L; // 幂等时间窗内的命令时间（同 run 内固定即可）
    ...
    @Test
    void balanceAdjustmentIdempotentOnSameTransactionId() {
        UserProfileService svc = new UserProfileService();
        svc.addEmptyUserProfile(UID);
        long extId = 9001L;

        assertThat(svc.balanceAdjustment(UID, USDT, 1000L, extId, NOW),
    ...
    // transactionId 不要求单调递增——外部乱序到达（如 chain reorg、MQ 重排）也应正常处理。
    @Test
    void balanceAdjustmentAcceptsOutOfOrderEvents() {
        UserProfileService svc = new UserProfileService();
        svc.addEmptyUserProfile(UID);

        assertThat(svc.balanceAdjustment(UID, USDT, 500L, 100L, NOW),
                is(CommandResultCode.SUCCESS));
        // ID 比上一条小，按"集合命中"语义仍应通过
        assertThat(svc.balanceAdjustment(UID, USDT, 300L, 50L, NOW),
                is(CommandResultCode.SUCCESS));
