...
class ITConservationFuzz {
     ...
     * （含降价至 underwater 走 LIF 接管），每步断言全局守恒。这是今年 scale bug 的直接回归护栏：
    ...
    public void fuzzM5LoanScaleMismatchConservation() throws Exception {
        ...
        try (final ExchangeTestContainer container = ExchangeTestContainer.create(
            ...
            container.submitCommandSync(ApiPoolDeposit.builder()
                    .shardId(0).currency(quoteCurrencyId).amount(500_000_000L).build(),
            ...
            for (int cycle = 0; cycle < numCycles; cycle++) {
                ...
                if (!hasLoan[b]) {
                    ...
                    container.submitCommandSync(ApiLoanCreate.builder()
                            .transactionId(ext++).uid(uid).loanId(uid).symbol(symbolId)
                ...
                if (action == 0) {
                    ...
                    container.submitCommandSync(ApiLoanRepay.builder()
                            .transactionId(ext++).uid(uid).loanId(uid).repayAmount(1_000L).build(),
                            CommandResultCode.SUCCESS);
                    repaid++;
                ...
                // 强平：action==1 用 50000（proceeds > debt → overpay）；action==2 砸到 5000（proceeds << debt → underwater 走 LIF 接管）。
            ...
            assertTrue(liqUnderwater > 0, "M5 未执行任何 underwater 强平（LIF 接管路径没被覆盖）");
