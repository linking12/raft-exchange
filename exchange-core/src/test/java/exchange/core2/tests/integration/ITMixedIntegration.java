...
//            container.createAskWithOrderId(80002L, UID_8, userSize, 8000L, spec.symbolId, MarginMode.CROSS);
     ...
     *   1) 空池 → 每 shard futuresInsuranceFund map 为空；
     *   2) 定向充值后，只有匹配 shard 的 futuresInsuranceFund 出现该 symbol entry；
    ...
    public void testInsuranceFundReportQueryPerShardBreakdown() throws Exception {
        ...
        try (final ExchangeTestContainer container = ExchangeTestContainer.create(getHignPerformanceConfiguration())) {
            ...
            // 1) 空池：byShard 包含所有 shard，但每 shard futuresInsuranceFund / LIF 均为空
            ...
            emptyReport.getByShard().forEachKeyValue((shardId, data) -> {
                assertThat("shard " + shardId + " futuresInsuranceFund should be empty",
                        data.getFuturesInsuranceFund().isEmpty(), is(true));
                assertThat("shard " + shardId + " LIF should be empty",
                        data.getLoanInsuranceFund().isEmpty(), is(true));
            });

            // 1b) 借贷 LIF 与期货 IF 同报表可见，且按 shard 分开
            container.submitCommandSync(ApiLoanIfDeposit.builder()
                    .shardId(0).currency(specA.quoteCurrency).amount(250_000L).build(), CommandResultCode.SUCCESS);

            InsuranceFundReportResult lifReport = container.getApi()
                    .processReport(new InsuranceFundReportQuery(), 102).get();
            assertThat(lifReport.getByShard().get(0).getLoanInsuranceFund().get(specA.quoteCurrency), is(250_000L));
            assertThat("LIF 是定向单 shard，其余 shard 不应被写入",
                    lifReport.getByShard().get(1).getLoanInsuranceFund().get(specA.quoteCurrency), is(0L));
            ...
            assertThat(shard0.getFuturesInsuranceFund().size(), is(1));
            InsuranceFundReportResult.FuturesIfEntry entryA0 = shard0.getFuturesInsuranceFund().get(symbolAId);
            ...
            assertThat(shard1.getFuturesInsuranceFund().size(), is(2));
            assertThat(shard1.getFuturesInsuranceFund().get(symbolAId).getAvailable(), is(300_000L));
            assertThat(shard1.getFuturesInsuranceFund().get(symbolBId).getAvailable(), is(700_000L));
            ...
            for (int s = 0; s < numShards; s++) {
                InsuranceFundReportResult.FuturesIfEntry entryA = report.getByShard().get(s).getFuturesInsuranceFund().get(symbolAId);
                if (entryA != null) totalSymbolAAvailable += entryA.getAvailable();
                InsuranceFundReportResult.FuturesIfEntry entryB = report.getByShard().get(s).getFuturesInsuranceFund().get(symbolBId);
