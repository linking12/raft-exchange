 ...
 *
 * <p>只覆盖 loan 相关 5 字段（利率归 LoanService 利率子系统，见 loan.md §3.5/§13；
 * Cross 抵押折价率是 CoreCurrencySpecification 币种级配置，不属于 symbol loanConfig）。
 */
class CoreSymbolSpecificationLoanConfigTest {
    ...
    @Test
    void updateLoanConfig_setsAll5FieldsAtomically() {
        CoreSymbolSpecification spec = freshDisabledSpec();

        spec.updateLoanConfig(6000, 8000, 7000, 1_000_000L, 90);
    ...
    void updateLoanConfig_snapshotRoundTrip_preservesUpdatedValues() {
        ...
        CoreSymbolSpecification spec = freshDisabledSpec();
        spec.updateLoanConfig(5000, 8500, 7500, 2_000_000L, 60);
    ...
    void updateLoanConfig_disable_zeroInitial_stateHashEqualsFreshDisabled() {
        ...
        enabled.updateLoanConfig(6000, 8000, 7000, 100L, 90);

        enabled.updateLoanConfig(0, 0, 0, 0, 0);
