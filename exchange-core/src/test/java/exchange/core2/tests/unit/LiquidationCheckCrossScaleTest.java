...
class LiquidationCheckCrossScaleTest {
    ...
    void checkCross_scaledMaintenanceTruncatesToZero_noDivideByZero() {
        ...
        le.setCommandSubmitter((cmd, onApplied) -> submitCount[0]++);
        ...
        // 截断仓的 totalMaintenanceMargin 贡献为 0 → warningThreshold=0，健康账户 equity≥0 走早退分支：不误强平
        assertEquals(0, submitCount[0], "缩放后归零的健康账户不应被误强平");
    }
}
