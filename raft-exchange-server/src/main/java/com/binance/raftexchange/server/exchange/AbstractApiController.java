...
public abstract class AbstractApiController {
    ...
    public static byte[] callExchange(ApiCommand apiCommand) throws Exception {
        ...
        //此时orderCommand是稳定的 相当于暂时从环上被占用 此时同步地进行thenApply确保序列化成byte[]后再出处理器范围
        //这样orderCommand就可以被安全地复用

        //不要阻塞地获取submitCommandAsyncFullResponse的结果，
        ...
        return api.submitCommandAsyncFullResponse(apiCommand) //submitCommandAsyncFullResponse 跟 非full版本接受的command一致
                .thenApply(SerializeHelper::serializeToCommandResult)
                .get();
    }
