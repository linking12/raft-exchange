...
| **强平触发 / 周期兜底扫** | on-lane(markprice/funding apply 即 targeted 检测)**+** 定时线程周期发 `LIQUIDATION_SCAN` 全量兜底(`LiquidationScheduledService`,默认 2s) | on-lane 同 Java;周期兜底**发令逻辑**在 `scheduler.rs::run_one_iteration`,已由 `ExchangeCore::tick_liquidation_scheduler`(外层定时器驱动、`set_scheduler_running` 门控)接入,产出的 `LIQUIDATION_SCAN`/`REPRICE_LOAN_RATES` 与 cascade 共用 `command_submitter`;**墙钟仍在外层**(库内不含线程,便于单测) | 主触发两侧相同=on-lane;周期兜底扫两侧都有,scan 与 FORCE/IF/ADL 走同一 submitter 出口。事件用全流多重集比,不比逐命令归属 |
| **清算命令提交 / 级联执行模型** | LiquidationEngine + LiquidationScheduledService 生成的 **scan/FORCE/IF/ADL** 经**同一** `ExchangeCore.setCommandSubmitter` → `api.submitCommand`:无 raft 直接提交、有 raft **经共识后**回流 apply | `ExchangeCore::set_command_submitter` **统一出口**,两版对齐:**未设(单节点)**=cascade 同步 FIFO 原地排空、scan 直接回灌 `process_command`;**设了(raft)**=cascade 尾 + `tick_liquidation_scheduler` 都把命令交给 submitter(外层过 raft 共识、提交后回流)。leader-gated 生成不变 | 提交模型两侧已对齐(统一 submitter,覆盖 scan/FORCE/IF/ADL)。**单节点** FIFO 顺序与 **raft** 共识顺序不同(前者会把 ADL origin 完全消耗后再跑其 stale ADL);但 R1 夹位 `normalizeCmdPositionSize`↔`normalize_cmd_position_size` **两侧逐字节一致**(`min(cmd.size, openVolume)`,`null`/`None` 分支都 `cmd.size=0`),故任何顺序都不造钱,终态一致(见 §7.5)。**raft submitter 实际实现(jraft/aeron)在外层 server,不在本 crate**;库内提供统一出口 + 单测分流(`raft_mode_hands_cascade_*` / `raft_mode_scheduler_tick_submits_scan_*` / `raft_mode_cascade_completes_across_rounds_*`)。命令 byte 码 `LIQUIDATION_SCAN` 两侧现均=44(Java 原 64 与 `LOAN_IF_DEPOSIT` 撞码,已改,便于上 raft 按码序列化) |
