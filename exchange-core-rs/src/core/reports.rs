...
mod tests {
    ...
    fn spot_trade_writes_back_mark_price() {
        ...
        assert_eq!(core.risk.last_price_cache.get(&SYMBOL).map(|r| r.mark_price), None, "挂单未成交不应回写");
        core.process_command(&mut OrderCommand {
            command: OrderCommandType::PlaceOrder,
            order_id: 11,
            symbol: SYMBOL,
        ...
        assert_eq!(core.risk.last_price_cache.get(&SYMBOL).map(|r| r.mark_price), Some(50), "现货成交价应回写 markPrice");
