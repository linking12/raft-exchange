...
//! - `testLiquidationTriggeredByHighLeverage` / `testLiquidationOfMaintenanceMargin`
//!   / `testLiquidationSendWarn` / `testLiquidationLeverage`：均需 `container.triggerLiquidation()`
//!   （提交 `LIQUIDATION_SCAN`/启动强平引擎）+ `updateCurrentPriceTo`，`ExchangeApi` 门面无对应公开方法 → 跳过。
//!
