...
pub struct OrderCommand {
    ...
    pub timestamp: i64,
    pub order_flags: i32,
    /// 对应 Java `OrderCommand.leverage`（`public int leverage = 1;`）。**Ruling P4-B**：Rust
    /// `#[derive(Default)]` 的零值是 `0` 而非 Java 字面初始值 `1`，两者下游语义等价——
    /// `SymbolPositionRecord::update_leverage`/`CoreSymbolSpecification::calculate_init_margin` 等消费点
    /// 都把 `leverage==0` 归一为 `1`（同 Java `updateLeverage(int)` 的 `0 -> 1` 规则），故保留 derive、
    /// 不手写 `Default` impl，不改变任何既有 `..Default::default()` 构造点行为。
    pub leverage: i32,
    ...
    /// `shard_recv_amount` 是 merge [`FundingFeeCommandProcessor::build_matcher_events`] 为本 shard 分配的
    /// 截断后金额（对应 Java `MatcherTradeEvent.price`/`matchedOrderUid` 一对；本移植不扩 `MatcherEventType`——
    /// Ruling P6-A，同 `internal_transfer_event`/`loan_reprice_events` 先例）。三元组均为原语类型
    /// （`BTreeMap<i64,i64>`/`i64`），不引入 processor 层类型依赖，保持 `common` 层对 `processors` 层零依赖
    /// （同 `internal_transfer_event` 用裸元组而非具名 struct 的理由）。单 shard 下（Ruling P6-C）
    /// `RiskEngine::settle_funding_fees_collect` 一次性完成 R1+merge 写入这里；`None` 表示"本命令没有可结算
    /// 事件"（`total_pay==0` 或 `total_recv_notional==0`，对应 Java `cmd.matcherEvent=null`）——R2
    ...
    /// 的返回值原样写入，R2 `RiskEngine::if_takeover_apply` 的 finalize 阶段读取它调用
    ...
    /// [`crate::core::processors::if_command_processor::IfCommandProcessor::build_matcher_event`] 文档）；
    /// `None` = 全拒（对应 Java `REJECT` 事件）。`RiskEngine::if_takeover_collect`（R1+merge 合并）写入，
    ...
    /// 全局最优序——单 shard 下"跨 shard best-of-N"退化为对这个列表的顺序消费，见
    /// `adl_command_processor.rs` 模块文档）。
    ///
    /// **这是 R2 finalize 释放 `pending_adl_size` 时必须读取的"原始 R1 表"**——`RiskEngine::adl_apply`
    /// 的 finalize 阶段用 `std::mem::take` 取走它整体消费一次（每个候选都对称释放，不管 [`Self::adl_events`]
    /// 里实际消费了多少），不会被 merge 污染（merge 只读它、产出一份独立的 `adl_events`，不改本字段——
    /// 对应 Java 注释"cursors 必须 clone，不能直接复用 cmd.adlUserPositionsByShard"，本移植用只读迭代 +
    ...
    /// "没有候选可减仓"）——R2 `RiskEngine::adl_apply` 用 `is_empty()` 判定是否跳过 taker 自身平仓（对应
    /// Java `matcherEvent.eventType != REJECT` 门）。`RiskEngine::adl_collect`（R1+merge 合并，单 shard 下
