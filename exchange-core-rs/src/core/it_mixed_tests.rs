...
//!            liquidationPrice / marginRatioScaleK / unrealizedProfit（report 聚合未建模）。needs harness extension。
...
//!         —— 依赖强平触发 + IF/ADL 接管 + 跨命令累计 fund event 计数/字段（LIQUIDATION_*、IF_*、ADL_*）。
//!   - testInsuranceFundDeposit* / testInsuranceFundWithdraw* / testInsuranceFundReportQuery*
//!         —— 依赖 ApiInsuranceFundDeposit/Withdraw + InsuranceFundReportQuery，ExchangeApi harness 未暴露。
//!   - testCrossMarginAlert_isolatedMarginExcluded / testCrossBpMultiPositionAllocation_regressionLock
//!         —— 依赖 MARGIN_ALERT / LIQUIDATION_ALERT fund event（强平扫描触发路径）。
//!
//! 已翻译（1 条）：testMixedExchangeAndMargin —— 期货挂单(pending) + 现货挂单联合 free/lock 校验（不含 fund event 断言）。
