...
impl Fixture {
    ...
    fn build(ob: Box<dyn IOrderBook>) -> Self {
        ...
        assert_eq!(fx.snapshot(), fx.exp.build(), "初始黄金盘口不一致");
        fx
    }
    ...
    fn newo(
        ...
        assert_eq!(rc, expect, "place order {id} 结果码不符");
        cmd
    }
    ...
    fn cancel(&mut self, id: i64, uid: i64, expect: CommandResultCode) -> OrderCommand {
        ...
        assert_eq!(rc, expect, "cancel {id} 结果码不符");
        cmd
    }
    ...
    fn reduce(&mut self, id: i64, uid: i64, size: i64, expect: CommandResultCode) -> OrderCommand {
        ...
        assert_eq!(rc, expect, "reduce {id} 结果码不符");
        cmd
    }
    ...
    fn update(&mut self, id: i64, uid: i64, price: i64, expect: CommandResultCode) -> OrderCommand {
        ...
        assert_eq!(rc, expect, "move {id} 结果码不符");
        cmd
    }
    ...
    fn clear(&mut self) {
        ...
        assert!(after.ask_prices.is_empty(), "clear 后仍有 ask");
        assert!(after.bid_prices.is_empty(), "clear 后仍有 bid");
    }
}
...
fn scn_should_match_ioc_order_full_liquidity(fx: &mut Fixture) {
    ...
    let orders = fx.ob.find_user_orders(UID_1);
    for gone in [1, 2, 3] {
        assert!(!orders.iter().any(|o| o.order_id == gone), "order {gone} 应已移除");
    }
}
